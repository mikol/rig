(function (global) {
  ...
  function AmdLoader() {
    if (AmdLoader.instance) {
      return AmdLoader.instance;
    }

    if (!(this instanceof AmdLoader)) {
      return new AmdLoader();
    }

    AmdLoader.instance = this;
  }
  
  AmdLoader.prototype.load = function (id, callback) {
    var el = document.createElement('script');
    el.charset = 'utf-8';
    el.async = true;
    el.src = id.toUri();
    el.onload = this.onLoad.bind(this, el, id, callback);

    sibling.parentNode.insertBefore(el, sibling);
  };

  AmdLoader.prototype.onLoad = function (el, id, callback) {
    var definitions = partials.slice()
      , dependencies = this.getDependencies(id.getDirname())
      ;
    ...
    require(dependencies, function () {
      while (definitions.length > 0) {
        var partial = definitions.shift()
          , uid = isFunction(partial.exporter) ? String(partial.exporter) : ''
          ;
  ...
  AmdLoader.prototype.getDependencies = function (opt_relativeTo) {
    var x = 0
      , xn = partials.length
      , result = []
      ;
    ...
    for (; x < xn; ++x) {
      var dependencies = partials[x].getDependencies();
  ...
  AmdLoader.prototype.addPartial = function (exporter, opt_dependencies, opt_id) {
    var uid = isFunction(exporter) ? String(exporter) : '';
    
    if (defining[uid] !== undefined || (opt_id && getModuleById(opt_id))) {
      return;
    }
    ...
    var definition = new AmdDefinition(exporter, opt_dependencies);
    
    if (opt_id) {
      definition.setId(opt_id);
    }
    
    partials.push(definition);
  };
   ...
   * can be completed, call {@code AmdDefinition.prototype.finalize}.
   *
   ...
   * @constructor
   */
  function AmdDefinition(exporter, opt_dependencies) {
   ...
   */
  AmdDefinition.prototype.getId = function () {
    return this.id;
  }

  /**
   * Adds the specified module ID to this definition and, as a side effect,
   * immediately exports the module's interface.
   */
  AmdDefinition.prototype.setId = function (id) {
    this.id = new AmdId(id);

    this.module.id = this.id.toString();
    this.module.uri = this.id.toUri().toString();
   ...
   */
  AmdDefinition.prototype.getDependencies = (function () {
    var commentRe = /(?:\/\*[\s\S]*?\*\/|\/\/.*)(?:[\n\r])*/g
   ...
   */
  AmdDefinition.prototype.getModules = function () {
  ...
  /**
   * Publishes this module's interface if it has an ID; otherwise does nothing.
   */
  AmdDefinition.prototype.export = function () {
    if (this.id) {
      cache[this.id] = this.exports;
    }
  };
  
  /**
   * Completes the initialization of this partial module. Which means that
   * after executing, and assuming this module has an ID, the module's
   * interface is available and ready for others to use.
   *
   ...
   *     file loads.
   */
  AmdDefinition.prototype.finalize = function (resourceId) {
    ...
    if (this.exporter) {
      ...
      this.exports = this.exporter.apply(this.exports, modules) || this.exports;
      this.export();
    }
  };
  
  // -------------------------------------------------------------------------
  // Asynchronous Module Definition (AMD) Utilities
  ...
  function getModules(dependencies) {
    ...
    for (; x < xn; ++x) {
      list.push(getModuleById(dependencies[x]));
    }
  ...
  function define(opt_id, opt_dependencies, exporter) {    
    ...
    new AmdLoader().addPartial(exporter, opt_dependencies, opt_id);
  }
  ...
  function requireAsync(dependencies, callback) {
    ...
    function fn() {
      if (xn === 0) {
        callback.apply(null, getModules(dependencies));
      }
  ...
  // rig.isArray = isArray;
  // rig.isFunction = isFunction;
  // rig.isObject = isObject;
  // rig.isString = isString;
